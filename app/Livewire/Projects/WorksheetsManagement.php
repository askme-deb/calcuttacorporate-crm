...
public function updatedAssignedUserId()
{
    $this->loadProjects();
    ...
    {
         $this->assignedUsers = User::orderBy('name')->get();
    ...
    public function loadProjects()
    {
        $user = auth()->user();
        $completedStatusId = WorkStatus::where('name', 'Completed')->value('id');
        $cancelledStatusId = WorkStatus::where('name', 'Cancelled')->value('id');
        $query = Worksheet::query()
            ->whereNotIn('status_id', [$completedStatusId, $cancelledStatusId])
            ->with([
                'client',
                'work',
                'priorty',
        ...
        if (!$user->can('View Projects')) {
            $query->where(function($q) use ($user) {
                $q->whereHas('projectTeamMembers', function ($q2) use ($user) {
        ...
        if (!empty($this->employee_id)) {
            $query->where(function($q) {
                $q->where('created_by', $this->employee_id)
                  ->orWhereHas('projectTeamMembers', function($tm) {
        ...
        if (!empty($this->overdue_filter)) {
            if ($this->overdue_filter == 'overdue') {
                $query->where('deadline', '<', now())->whereHas('projectStatus', function($q) {
                    $q->where('name', '!=', 'Completed');
                });
            } elseif ($this->overdue_filter == 'pending') {
                $query->whereHas('projectStatus', function($q) {
                    $q->where('name', '!=', 'Completed');
                });
            } elseif ($this->overdue_filter == 'completed') {
                $query->whereHas('projectStatus', function($q) {
                    $q->where('name', 'Completed');
                });
            }
        ...
        // Deadline filter
        if (!empty($this->deadline_from)) {
            $query->whereDate('deadline', '>=', $this->deadline_from);
        }
        if (!empty($this->deadline_to)) {
...
// Export Excel
    ...
    {
        $this->loadProjects();
        return Excel::download(new \App\Exports\ProjectsExport($this->worksheets), 'projects.xlsx');
    }
    ...
    {
        $this->loadProjects();
        $pdf = Pdf::loadView('exports.projects', ['projects' => $this->worksheets])
