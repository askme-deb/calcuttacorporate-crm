...
class MeetingSummaryManagement extends Component
{
    ...
    {
        $this->clients = Client::orderBy('client_name')->get(['id', 'client_name', 'phone_number'])->toArray();
        $this->employees = User::orderBy('name')->get(['id', 'name'])->toArray();
        $this->resetMeeting();
    }
    ...
    public function render()
    {
        ...
        if ($routeName === 'meetings.summary.edit') {
            // Dedicated edit page
            $meetingId = $route->parameter('meeting');
            if ($meetingId && $meetingId != $this->editingMeetingId) {
                $this->editMeeting((int)$meetingId);
            }
            return view('livewire.meetings.meeting-summary-form', [
                'meetingId' => $meetingId,
            ]);
    ...
    public function save(): void
    {
        ...
        $message = $this->editingMeetingId ? 'Meeting summary updated successfully.' : 'Meeting summary created successfully.';
    ...
    public function updatedMeetingClientId($value): void
    {
        ...
        if (empty($this->meeting['client_name'])) {
            $this->meeting['client_name'] = $client->client_name;
        }

        if (empty($this->meeting['contact_number'])) {
