                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-body mb-n3">
                            <button class="btn btn-outline-primary btn-sm px-4 mt-0 mb-3" wire:click="addInstitute()" type="button" >
                                <span wire:loading wire:target="addInstitute">
                                    <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                </span>
                                <span wire:loading.remove wire:target="addInstitute">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($institutes as $ins)
                                            <tr>
                                                <td>{{$i++}}</td>
                                                <td> {{ $ins->name }}</td>
                                                <td> {!! getStatus($ins->is_visible) !!}</td>
                                                <td> {{ $ins->created_at }}</td>

                                                <td class="text-end">
                                                    <a href="javascript:;" wire:click="edit({{ $ins->id }})"><i class="las la-pen text-secondary font-16 text-info"></i></a>
                                                    <a href="javascript:;" onclick="confirmDeletion({{ $ins->id }})"><i class="las la-trash-alt text-secondary font-16 text-danger"></i></a>
                                                </td>
                                            </tr>
                                        @endforeach
                                    </tbody>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title m-0" id="exampleModalDefaultLabel"> {{ $modalMode === 'edit' ? 'Edit Institute' : 'Cretae Institute' }}</h6>
            ...
            @else
                <form wire:submit.prevent="createInstitute" class="needs-validation">
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-12">
                    ...
                    <div class="modal-footer">
                        <button type="button" wire:click="closeModal" class="btn btn-outline-secondary btn-sm" data-bs-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-outline-primary btn-sm">
                            <span wire:loading wire:target="createInstitute">
                                <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Loading...
                            </span>
                            <span wire:loading.remove wire:target="createInstitute">
                                Save changes
                            </span>
                        </button>
                    </div><!-- end modal-footer -->
